...
export default function Header({ dark, setDark, user, onLogin, onLogout, credits, onSubscribe }) {
  ...
  const navTo = (id) => { navigate(ROUTE_MAP[id]); setMobileOpen(false); };
  ...
  return (
    ...
    >
      <div className="h-full flex items-center justify-between gap-3 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          {user ? (
            ...
            <button
              onClick={onLogin}
              className="hidden sm:flex items-center gap-2 text-[13px] font-bold px-5 py-2 rounded-lg transition-all duration-150 shadow-sm hover:scale-[1.02] active:scale-95"
          ...
          <button
            ...
            className={`md:hidden p-2 rounded-lg border transition-all duration-150 flex items-center justify-center
              ${dark
                ? "bg-stone-800/50 border-stone-700 text-stone-400 hover:bg-stone-700 hover:text-stone-200"
                : "bg-white border-stone-200 text-stone-500 hover:bg-stone-50 hover:text-stone-700 shadow-sm"
              }`}
      ...
      {/* ── Mobile Menu ─────────────────────────────────────────────────── */}
      {mobileOpen && (
        <div className={`md:hidden absolute top-[64px] left-0 right-0 z-[99] border-b shadow-lg
          ${dark
            ? "bg-[rgba(5,5,5,0.97)] border-[#1a1a1a]"
            : "bg-white/97 border-stone-200"
          }`}
        >
          <div className="px-4 py-3 flex flex-col gap-1">
            {NAV_ITEMS.map((n) => (
              <button
                ...
                style={{
                  color: ROUTE_MAP[n.id] === location.pathname ? "#F5A623" : (dark ? "#ccc" : "#444"),
                }}
              >
                <span>{n.label}</span>
                {n.pro && (
            ...
            {/* Divider */}
            <div className={`my-2 h-px ${dark ? "bg-stone-800" : "bg-stone-100"}`} />

            {/* Auth row */}
            {user ? (
              <>
                <div className={`px-3 py-2 text-xs ${dark ? "text-stone-500" : "text-stone-400"}`}>
                  Signed in as <span className={`font-medium ${dark ? "text-stone-300" : "text-stone-700"}`}>{user.name?.split(" ")[0]}</span>
                  {user.isPro && <span className="ml-2 text-violet-400 font-semibold">· Pro</span>}
                </div>
                {!user.isPro && (
                  <button
                    onClick={() => { onSubscribe(); setMobileOpen(false); }}
                    className="mx-3 mb-1 py-2.5 rounded-lg text-sm font-bold transition-colors"
                    style={{ background: "#F5A623", color: "#000" }}
                  >
                    Upgrade to Pro — ₹99/mo
                  </button>
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-3 rounded-lg text-sm font-medium transition-colors
                    ${dark ? "text-stone-400 hover:bg-stone-800 hover:text-stone-200" : "text-stone-500 hover:bg-stone-50 hover:text-stone-700"}`}
                >
                  Sign out
                </button>
              </>
              ...
              <button
                ...
                className="mx-3 mb-2 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-bold transition-all"
                style={{ background: "#F5A623", color: "#000" }}
              >
                <svg className="w-3.5 h-3.5" viewBox="0 0 24 24">
                  <path fill="#000" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
